...
def generate_cases(count: int) -> list[ReportCase]:
    ...
    for index in range(count):
        ...
        focus = METRICS[index % len(METRICS)]
        factor = 1.28 if index % 2 == 0 else 0.68
        current[focus] = round(previous[focus] * factor, 2)
        current = {name: round(value, 2) for name, value in current.items()}
        truth = primary_change(previous, current)
        cases.append(ReportCase(f"REPORT-{index:03d}", previous, current, truth))
    return cases


def ask_ollama(batch: list[ReportCase], model: str, url: str) -> tuple[dict[str, dict[str, object]], dict[str, int]]:
    items = [
        {
            "case_id": case.case_id,
            "previous": case.previous,
            "current": case.current,
            "change_percent": {name: round((case.current[name] - case.previous[name]) / case.previous[name] * 100, 2) for name in METRICS},
    ...
    prompt = (
        ...
        "must contain case_id, metric, direction (up or down), previous, current, summary, and statement_type=verified_fact. "
        "Copy previous and current values exactly from the selected metric. Do not invent numbers. Do not omit cases.\n\n" + json.dumps(items)
    )
    payload = {"model": model, "stream": False, "format": "json", "keep_alive": "10m", "options": {"temperature": 0, "num_predict": 1700}, "messages": [{"role": "system", "content": "You analyze locked KPI data. Output grounded JSON only."}, {"role": "user", "content": prompt}]}
...
def run(count: int, model: str, url: str, batch_size: int) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    for case in cases:
        ...
        direct = bool(proposal and proposal.get("metric") == case.truth["metric"] and proposal.get("direction") == case.truth["direction"] and float(proposal.get("previous", -1)) == case.truth["previous"] and float(proposal.get("current", -1)) == case.truth["current"])
        outcome = analyze_and_publish(case.previous, case.current, proposal)
        approved = outcome.approved_for_publication and outcome.analysis["metric"] == case.truth["metric"] and outcome.analysis["previous"] == case.truth["previous"] and outcome.analysis["current"] == case.truth["current"]
        rows.append({"case_id": case.case_id, "ai_direct_pass": direct, "decision_source": outcome.source, "metric": outcome.analysis["metric"], "numbers_grounded": approved, "approved": approved})
    elapsed = time.perf_counter() - started
    direct = sum(row["ai_direct_pass"] for row in rows)
    approved = sum(row["approved"] for row in rows)
    summary = {
        "benchmark": "autonomous_kpi_analysis_v1", "live_model": True, "model": model,
