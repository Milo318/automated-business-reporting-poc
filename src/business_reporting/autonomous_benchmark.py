...
def generate_cases(count: int) -> list[ReportCase]:
    ...
    for index in range(count):
        ...
        cases.append(ReportCase(f"REPORT-{index:03d}", previous, current, truth, challenge))
    return cases
    ...
    items = [
        {
            ...
            "change_percent": {name: round((case.current[name] - case.previous[name]) / max(abs(case.previous[name]), 1.0) * 100, 2) for name in METRICS},
...
def run(count: int, model: str, url: str, batch_size: int) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    for case in cases:
        ...
        rows.append({"case_id": case.case_id, "challenge": case.challenge, "ai_direct_pass": direct, "decision_source": outcome.source, "metric": outcome.analysis["metric"], "numbers_grounded": approved, "approved": approved})
    ...
    summary = {
        "benchmark": "autonomous_kpi_analysis_v2_stress", "live_model": True, "model": model,
