...
def main() -> None:
    ...
    parser.add_argument("--previous", type=Path, default=Path("data/mock/previous_metrics.json"))
    ...
    if args.ai:
        ...
        outcome = analyze_and_publish(previous, current, propose_analysis(primary_change(previous, current)))
        analysis = outcome.to_dict()
        summary = str(outcome.analysis["summary"])
    render_report(metrics, args.output, summary)
    result = {"synthetic_data": True, "metrics": metrics.to_dict(), "reconciliation": reconciliation(metrics)}
    if analysis:
